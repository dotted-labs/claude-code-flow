...
export class InitCommand {
  ...
  private async planActions(): Promise<InitSummary> {
    ...
    const commandsDestDir = path.join(
      this.options.cwd,
      ".claude",
      "commands",
      "dl"
  ...
  private async copyAgentsDirectory(): Promise<CopyResult[]> {
    ...
    if (!(await fs.pathExists(srcDir))) {
      console.log(
        "  ⚠️  No agents directory found in package assets, skipping"
      );
      return [];
    }
  ...
  private async copyCommandsDirectory(): Promise<CopyResult[]> {
    ...
    const destDir = path.join(this.options.cwd, ".claude", "commands", "dl");

    if (!(await fs.pathExists(srcDir))) {
      console.log(
        "  ⚠️  No commands directory found in package assets, skipping"
      );
      return [];
    }
  ...
  private async mergeMcpConfig(): Promise<{
    ...
    const packageMcpConfig =
      await readJsonWithBackup<McpConfig>(packageMcpPath);
    if (!packageMcpConfig) {
    ...
    if (result.action === "created") {
      ...
      console.log(
        `  ✅ Updated mcp.json, added ${result.entriesAdded} new entries`
      );
  ...
  private async updateClaudeMd(): Promise<{
    action: "created" | "updated" | "skipped";
    ...
    const result = await insertOrUpdateSectionBySentinels(
      claudeMdPath,
      promptContent.trim()
    );
  ...
  private logCopyResults(results: CopyResult[], type: string): void {
    ...
    if (conflicts.length > 0) {
      console.log(
        `  ⚠️  ${conflicts.length} ${type} file conflicts (use --force to overwrite):`
      );
      conflicts.forEach((c) =>
        console.log(`    - ${path.relative(this.options.cwd, c.path)}`)
      );
    }
  }
